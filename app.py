...
def extraer_monto(texto):
    ...
    # Fallback: mayor número del ticket excluyendo líneas de impuesto
    mayor = 0
    for linea in lineas:
        if any(ex in linea for ex in keywords_excluir):
            continue
...
def extraer_descripcion(texto):
    import re
    lineas = texto.split("\n")
    # Saltear líneas con info irrelevante (fechas, tel, números largos, URLs)
    skip_patterns = [
        r"^\d{2,4}[-/年]\d{1,2}",  # fechas
        r"^\d{10,}",                  # números largos (tel, códigos)
        r"http",                        # URLs
        r"^T\d{13}",                  # número de registro fiscal
        r"^\d+$",                     # solo números
        r"登録番号",
        r"ありがとう",
        r"お問合",
    ...
    for linea in lineas:
        ...
        if len(linea) < 2:
            continue
        if any(re.search(p, linea) for p in skip_patterns):
            continue
        return linea[:50]
    return "Ticket"
